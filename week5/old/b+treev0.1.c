...
Node **search_Range(Node *x, int s, int e) {
    if (x->leaf == 1) {                                                       //x가 리프노드일경우
        ...
        for (int j = 1; j <= x->link->N; j++) {
            if (x->link->key[j] > e) {
                break;
            }
            array[j] = x->link->key[j];
        }
    } else {
        int i = 1;
    ...
    while (1) {
        ...
        while (1) {
            ...
            if (command == 1) {
                ...
                Node **searched_array = search_Range(tree->root, startNum, endNum);
